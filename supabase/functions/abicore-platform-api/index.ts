...
// Structured error response helper for AbiVC integration
...
// Flexible interface to handle AbiVC's actual structure
interface IntakeSubmission {
  field_label: string;
  field_type: string; // AbiVC sends: required_info, long_text, linkedin, checkbox, multiple_choice, short_text, number, etc.
...
interface SyncCompanyPayload {
  ...
  intake_submissions?: IntakeSubmission[]; // May be here or inside company_data
  synced_at: string;
  ingest_point?: string; // NEW: 'initial_submission', 'general_submission', 'general_platform_transformation', etc.
}

interface PlatformRequest {
  action: "health" | "ping" | "sync_company" | "get_status" | "get_ingest_points";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const expectedSecret = Deno.env.get("ABIVC_PLATFORM_SECRET");
    ...
    const validActions = ["health", "ping", "sync_company", "get_status", "get_ingest_points"];
    ...
    console.log(`Processing action: ${body.action} at ${now}`);
    ...
    switch (body.action) {
      ...
      case "get_status":
        return new Response(
          JSON.stringify({
            ...
            platform: "abicore",
            last_check: now,
            capabilities: ["health", "ping", "get_status", "sync_company", "get_ingest_points"],
          }),
          { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } }
        );

      case "get_ingest_points": {
        // Return available ingest points for AbiVC to select from
        const { data: ingestPoints, error: ingestError } = await supabase
          .from('integration_ingest_sources')
          .select('ingest_point_id, name, description, fields')
          .eq('integration_id', 'abivc')
          .eq('is_active', true)
          .order('sort_order');

        if (ingestError) {
          console.error("Failed to fetch ingest points:", ingestError);
          return createErrorResponse(
            "Failed to fetch ingest points",
            {
              code: "DATABASE_ERROR",
      ...
      case "sync_company": {
        ...
        const { company_uuid, company_data, synced_at, ingest_point } = payload!;
        
        // Get intake_submissions - AbiVC sends it INSIDE company_data, not as separate field
        const intake_submissions = company_data.intake_submissions || payload!.intake_submissions || [];
        
        // Default ingest_point to 'initial_submission' for backward compatibility
        const targetIngestPoint = ingest_point || 'initial_submission';
        console.log(`Targeting ingest point: ${targetIngestPoint}`);
        
        console.log(`Syncing company: ${company_data.name} (${company_uuid})`);
        console.log(`Intake submissions count: ${intake_submissions?.length || 0}`);
        console.log(`Intake submissions sample:`, JSON.stringify(intake_submissions?.slice(0, 2)));
        ...
        const { data: company, error: companyError } = await supabase
          ...
          .upsert({
            ...
            status: 'active',
            metadata: {
              abivc_organization_id: company_data.organization_id,
        ...
        if (intake_submissions && Array.isArray(intake_submissions)) {
          ...
          for (const submission of intake_submissions) {
            // Extract actual value - AbiVC wraps it in { value: ... }
          ...
          console.log(`Processed ${Object.keys(intakeFields).length} intake fields`);
        }
        ...
        const { data: submission, error: submissionError } = await supabase
          ...
          .insert({
            ...
            raw_data: rawData,
            source_type: 'abivc_sync',
            status: 'pending',
            metadata: { 
              synced_from: 'abivc',
              synced_at: synced_at || now,
              ingest_point: targetIngestPoint,
        ...
        try {
          ...
          console.log("Workflow execution result:", JSON.stringify(workflowResult));
        } catch (workflowError) {
          console.error("Workflow execution failed:", workflowError);
          // Don't fail the sync - data is saved, workflows can be retried
        }

        // Return success response with data wrapper for AbiVC
        return new Response(
          JSON.stringify({
            success: true,
