...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`User ${userId} testing platform connection`);
    ...
    // Get the platform secret
    const platformSecret = Deno.env.get("ABIVC_PLATFORM_SECRET");
    if (!platformSecret) {
      console.error("ABIVC_PLATFORM_SECRET not configured");
      return new Response(
    ...
    // Call the actual platform API
    const platformApiUrl = `${supabaseUrl}/functions/v1/abicore-platform-api`;
    console.log(`Calling platform API: ${body.action}`);
    ...
    if (!platformResponse.ok) {
      console.error("Platform API error:", platformData);
      return new Response(
    ...
    console.log(`Platform API ${body.action} succeeded`);
    return new Response(
      JSON.stringify(platformData),
