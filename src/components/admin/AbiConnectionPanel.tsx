...
export function AbiConnectionPanel() {
  ...
  const { latestHealth, alerts, refreshHealth, resolveAlert } = useIntegrationHealth('abi');
  ...
  const callPlatformAPI = async (action: string, payload?: any) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('test-abi-connection', {
  ...
  const testHealth = async () => {
    ...
    if (result.status === 'success') {
      ...
      toast({ title: 'Health Check Passed', description: 'Abi platform is healthy' });
    } else {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Abi Platform Connection
          </CardTitle>
          <CardDescription>
            Test and monitor the connection to the Abi platform
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <IngestSourcesSection 
        integrationId="abi"
        integrationName="Abi"
        integrationColor="hsl(160 84% 39%)"
      />
    </div>
  );
}
